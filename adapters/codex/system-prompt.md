...
CLI 工具位置：`{安装路径}/media-topic-skill/core/topic.js`
...
- 如果文件不存在 → 运行 `node topic.js init`
- 如果 `creatorProfile.niche` 为空 → 从用户输入中提取画像，后续自动创建
- 如果已有配置 → 直接使用

### 第二步：处理笔记

运行 `node topic.js ingest "用户输入"` 获取分析 prompt，然后按 prompt 要求完成：
1. 去重分析 — 对比已有选题，判断 MERGE 或 NEW
2. 主题分类 — 优先使用已有主题名
...
node topic.js batch '{
  ...
  "topics": [{ "raw": "...", "title": "...", "theme": "...", "platform": "...", "format": "...", "stage": "...", "tags": [...], "priority": "...", "viral": 85, "brand": 70, "biz": 60, "comp": 45, "recommend": "...", "viralReason": "✓理由|✗理由", "brandReason": "...", "bizReason": "...", "compReason": "..." }],
  "merges": [[目标ID, 源ID]]
}'
```

- `profile` 只在首次需要传入
- batch 命令自动完成：保存 + 合并 + 生成看板
